...
channel_access_token = "JR7FA18822eZSihJGZbatQd1ma8LOOUCCjL1VbLFtZRBdh6c0v3YjBuRDmgCIiDR/yycTDNUV1Tc6PfqVWl0HPXutWI/aCG4xbf48ARBZwBw8Y3S6divLwfPIEJfuLKheFdHWdwHphKfT6OzKWmo7gdB04t89/1O/"
...
def send_game_info(reply_token, yacht_dice, player ,update_score = False):
    ...
    if update_score:
        ...
        yacht_dice.reply_json["contents"][0]["body"]["contents"][2 * the_other_player + 1]["contents"][3]["text"] = str(the_other_player_bonus_sum) + " / 63"
        yacht_dice.reply_json["contents"][0]["body"]["contents"][2 * the_other_player + 1]["contents"][5]["text"] = str(the_other_player_total)
        ...
        yacht_dice.reply_json["contents"][0]["body"]["contents"][2 * player + 1]["contents"][1]["color"] = "#F08734"
        yacht_dice.reply_json["contents"][0]["body"]["contents"][2 * the_other_player + 1]["contents"][1]["color"] = None
        for i in range(1, 13):
    ...
    for i in range(0, 5):
        yacht_dice.reply_json["contents"][0]["footer"]["contents"][1]["contents"][i]["url"] = \
